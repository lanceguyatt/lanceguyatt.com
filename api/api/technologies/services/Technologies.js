...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Technologies
      .findOne(_.pick(params, _.keys(Technologies.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Technologies.create(data);

    // Create relational data and return the entry.
    return Technologies.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Technologies.associations.map(a => a.alias));
    const data = _.omit(values, Technologies.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Technologies.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Technologies.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Technologies.attributes).reduce((acc, curr) => {
      switch (Technologies.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
